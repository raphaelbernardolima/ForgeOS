  ...
  return (
    <div id="simulador-section" className="w-full py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 bg-[#12151d] border border-neutral-800 rounded-2xl p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              {(['portao', 'grade', 'corrimao', 'guarda-corpo'] as TipoPeca[]).map(t => (
                <button
                  ...
                  className={`py-2 px-3 rounded-lg text-xs font-medium capitalize transition-all border text-left flex items-center justify-between ${
                    tipo === t
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2 text-sm text-white font-mono focus:border-amber-500 focus:outline-none tabular-nums"
                />
              </div>
              <div>
          ...
          <div>
            <label className="text-xs font-mono uppercase tracking-wider text-neutral-400 block mb-2 font-medium">
              3. Família Matemática de Desenho
            </label>
            <div className="grid grid-cols-2 gap-2">
          ...
          <div className="space-y-4 pt-2 border-t border-neutral-800">
            ...
            <div>
              <div className="flex justify-between text-xs font-mono text-neutral-300 mb-1">
                <label htmlFor={assimetriaId} className="cursor-pointer">Assimetria de ritmo</label>
            ...
            {/* Amplitude (para ondulado ou geométrico) */}
            {(genes.familia === 'ondulado' || genes.familia === 'geometrico') && (
              <div>
                <div className="flex justify-between text-xs font-mono text-neutral-300 mb-1">
                  <label htmlFor={amplitudeId} className="cursor-pointer">Amplitude de curvatura</label>
                  <span className="text-amber-400 tabular-nums">{genes.amplitude}</span>
                </div>
                <input
                  id={amplitudeId}
                  type="range"
                  ...
                  max={40}
                  step={1}
                  value={genes.amplitude}
                  onChange={(e) => handleGeneChange('amplitude', parseFloat(e.target.value))}
                  className="w-full accent-amber-500 cursor-pointer"
                />
              </div>
            )}

            {/* Frequência (para ondulado ou geométrico) */}
            {(genes.familia === 'ondulado' || genes.familia === 'geometrico') && (
              <div>
                <div className="flex justify-between text-xs font-mono text-neutral-300 mb-1">
                  <label htmlFor={frequenciaId} className="cursor-pointer">Frequência de repetição</label>
                  <span className="text-amber-400 tabular-nums">{genes.frequencia}</span>
                </div>
                <input
                  id={frequenciaId}
                  type="range"
                  ...
                  max={20}
                  step={0.5}
                  value={genes.frequencia}
                  onChange={(e) => handleGeneChange('frequencia', parseFloat(e.target.value))}
                  className="w-full accent-amber-500 cursor-pointer"
                />
              </div>
            )}
                  ...
                  <div className="grid grid-cols-2 gap-1.5 pt-1">
                    {(['tubular', 'industrial', 'cantoneira', 'minimalista'] as EstiloMoldura[]).map(est => (
                      <button
                        ...
                        className={`text-[11px] py-1 px-2 rounded border font-mono capitalize text-center ${
              ...
              {CORES_PINTURA.map((c, i) => (
                <button
                  ...
                  className={`p-2 rounded-lg border text-left flex flex-col gap-1 transition-all ${
                    corIndex === i
                      ? 'border-amber-400 bg-neutral-800/80 shadow'
            ...
            <div className="p-3.5 rounded-xl border border-amber-500/20 bg-gradient-to-r from-amber-500/5 to-transparent flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-[11px] text-neutral-400 mt-0.5 line-clamp-1">
                  Guarde este padrão ({genes.familia}, {genes.espessura}cm) para reaproveitar em futuros orçamentos.
                </p>
              </div>
              <button
                type="button"
                ...
                className="shrink-0 flex items-center gap-1.5 px-3 py-2 bg-amber-500 hover:bg-amber-400 text-neutral-950 font-bold rounded-lg text-xs font-mono transition-all shadow-md active:scale-95 whitespace-nowrap"
              >
                <Bookmark className="w-3.5 h-3.5" />
